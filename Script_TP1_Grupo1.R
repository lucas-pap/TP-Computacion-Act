...
info_c_location<-function(x){
  if(company_location[x,"n"]>10)
  {
    a<-company_location[x,"Media"]
  }else
  {
    a<-mean(cacao$Rating)
  }
...
info_c_location(cacao$CompanyLocation)


# Para la variable Broad Bean Origin

info_broad_bean<-function(y){
  if(broad_bean[y,"n"]>10)
  {
    b<-broad_bean[y,"Media"]
  }else{
    b<-mean(cacao$Rating)
  }
  return(b)
}
info_broad_bean(cacao$BroadBeanOrigin)
